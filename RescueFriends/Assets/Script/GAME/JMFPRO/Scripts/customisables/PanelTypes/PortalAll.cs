...
public class PortalAll : PanelDefinition {
    ...
    void OnGameReady () {
        ...
        foreach (Board bdB in GM.Boards) {
            if ((bdB.PND is PortalAll) == false) continue;

            portalDict.Add(bdB, bdB);
        }
    }

    public Board GetOtherSide (Board board) {
        Debugger.Assert(portalDict.ContainsKey(board), "PortalAll.GetOtherSide : Can not find portal board.");
