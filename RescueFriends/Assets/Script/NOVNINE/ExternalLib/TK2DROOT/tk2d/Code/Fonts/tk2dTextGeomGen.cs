...
public static class tk2dTextGeomGen
{
    ...
    static int HandleStyleCommand(string cmd)
    {
        ...
        switch (cmdSymbol) {
        case 'c':
            cmdLength = 1 + SetColorsFromStyleCommand(cmdArgs, false, false);
            break;
        case 'C':
            cmdLength = 1 + SetColorsFromStyleCommand(cmdArgs, false, true);
            break;
        case 'g':
            cmdLength = 1 + SetColorsFromStyleCommand(cmdArgs, true, false);
            break;
        case 'G':
            cmdLength = 1 + SetColorsFromStyleCommand(cmdArgs, true, true);
            break;
        }
...
#else
    public static int SetTextMeshGeom(Vector3[] pos, Vector2[] uv, Vector2[] uv2, Color32[] color, int offset, GeomData geomData, float italic = 0f)
#endif
    {
        tk2dTextMeshData data = geomData.textMeshData;
...
#if !TK2D_ORIGINAL
            float height = chr.p0.y - chr.p1.y;
            pos[offset + target * 4 + 0].x += height * italic * 0.5f;
            pos[offset + target * 4 + 1].x += height * italic * 0.5f;
            pos[offset + target * 4 + 2].x -= height * italic * 0.5f;
            pos[offset + target * 4 + 3].x -= height * italic * 0.5f;
#endif
