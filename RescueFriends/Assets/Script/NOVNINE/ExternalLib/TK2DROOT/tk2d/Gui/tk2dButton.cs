...
public class tk2dButton : MonoBehaviour
{
...
#else
    public IEnumerator coHandleButtonPress(int fingerId = -1)
#endif
    {
...
#if !TK2D_ORIGINAL
#if UNITY_3_5
        if (false == viewCamera.enabled || false == viewCamera.gameObject.active)
            return;
#else
        if (false == viewCamera.enabled || false == viewCamera.gameObject.activeInHierarchy)
            return;
#endif
#endif
    ...
    {
        get {
            return buttonDownSprite;
        } set {
            buttonDownSprite = value;
    ...
    {
        get {
            return buttonPressedSprite;
        } set {
            buttonPressedSprite = value;
    ...
    public string spriteName
    {
        set {
            ...
            this.ButtonDownSprite = value;
            this.ButtonUpSprite = value;
            this.ButtonPressedSprite = value;
        }
    }

    public static tk2dButton HitTest(Camera camera, Vector3 mousePosition)
    {
        Ray ray = camera.ScreenPointToRay(mousePosition);
        return HitTest(ray);
    }
    public static tk2dButton HitTest(Ray ray)
    {
        tk2dButton button = null;
        RaycastHit hitInfo;
        if (true == Physics.Raycast(ray, out hitInfo)) {
            button = hitInfo.collider.gameObject.GetComponent<tk2dButton>();
        }
        return button;
    }

    public static tk2dButton[] HitTestAll(Camera camera, Vector3 mousePosition)
    {
        Ray ray = camera.ScreenPointToRay(mousePosition);
        return HitTestAll(ray);
    }
    public static tk2dButton[] HitTestAll(Ray ray)
    {
        RaycastHit[] hitInfos = Physics.RaycastAll(ray);

        System.Collections.Generic.List<tk2dButton> listButton = new System.Collections.Generic.List<tk2dButton>();
        for (int i = 0; i < hitInfos.Length; i++) {
            tk2dButton button = hitInfos[i].collider.gameObject.GetComponent<tk2dButton>();
            if (null != button) {
                listButton.Add(button);
            }
        }

        tk2dButton[] items = listButton.ToArray();
        listButton.Clear();
        return items;
    }
