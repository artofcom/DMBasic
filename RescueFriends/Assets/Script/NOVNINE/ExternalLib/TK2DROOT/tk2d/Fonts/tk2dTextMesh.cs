...
#if !TK2D_ORIGINAL
    [SerializeField] float italic = 0.0f;
#endif
...
#if USE_BMPlatform
        //amugana
        if(!string.IsNullOrEmpty(_source) && _source[0] == '@')
            _source = Locale.GetString(_source);
        //amugana
#endif
#endif
...
#else
            int target = tk2dTextGeomGen.SetTextMeshGeom(vertices, uvs, uv2, untintedColors, 0, geomData, italic);
#endif
