...
public class TextureEditor
{
    ...
    {
        get {
            return SpriteCollection.editorDisplayScale;
        } set {
            SpriteCollection.editorDisplayScale = value;
        }
    }
    ...
    Color handleInactiveColor
    {
        get {
            return _handleInactiveColors[(int)currentColliderColor];
        }
    }
    Color handleActiveColor
    {
        get {
            return _handleActiveColors[(int)currentColliderColor];
        }
    }
    ...
    void HandleKeys()
    {
        ...
        if (evt.type == EventType.KeyUp && evt.shift) {
            ...
            switch (evt.keyCode) {
            case KeyCode.Q:
                newMode = Mode.Texture;
                break;
            case KeyCode.W:
                newMode = Mode.Anchor;
                break;
            case KeyCode.E:
                newMode = Mode.Collider;
                break;
            case KeyCode.R:
                newMode = Mode.AttachPoint;
                break;
            case KeyCode.N:
                drawColliderNormals = !drawColliderNormals;
                HandleUtility.Repaint();
                break;
            }
        ...
        if (texture == null) {
            ...
            if (mode == Mode.AttachPoint) {
                ...
                foreach (tk2dSpriteDefinition.AttachPoint ap in param.attachPoints) {
                    ...
                    Handles.DrawLine(pos, pos - Rotate(Vector2.up, ap.angle) * handleSize);
                    Handles.color = Color.red;
                    Handles.DrawLine(pos, pos + Rotate(Vector2.right, ap.angle) * handleSize);
