...
public class TimelineEditor
{
    ...
    Rect GetRectForTrigger(Rect triggerRect, int frame)
    {
        return new Rect(triggerRect.x + clipLeftHeaderSpace + frameWidth * frame - 3, triggerRect.y + 1, 15, 14);
    }
    int GetRoundedSelectedTrigger(Rect triggerRect, Vector2 mousePosition)
    {
        return (int)Mathf.Round((mousePosition.x - triggerRect.x - clipLeftHeaderSpace) / frameWidth);
    }
    ...
    Rect GetRectForFrame(Rect fgRect, int frame, int numFrames)
    {
        return new Rect(fgRect.x + clipLeftHeaderSpace + frameWidth * frame, fgRect.y, numFrames * frameWidth, fgRect.height);
    }
    ...
    {
        get {
            if (tk2dPreferences.inst.animFrameWidth == -1) return 80;
            else return Mathf.Clamp(tk2dPreferences.inst.animFrameWidth, minFrameWidth, maxFrameWidth);
        } set {
            tk2dPreferences.inst.animFrameWidth = value;
        }
    }
