...
public class tk2dSpritePickerPopup  : EditorWindow
{
    class PresentationParams
    {
        ...
        {
            get {
                return tk2dPreferences.inst.spriteThumbnailSize;
            } set {
                tk2dPreferences.inst.spriteThumbnailSize = Mathf.Clamp(value, tileSizeMin, tileSizeMax);
            }
        }
    ...
    void HandleKeyboardShortcuts()
    {
        ...
        if (ev.type == EventType.KeyDown) {
            ...
            case KeyCode.DownArrow:
                if (newSelectedIndex + numTilesPerRow < selectedDefinitions.Count) newSelectedIndex += numTilesPerRow;
                break;
            case KeyCode.UpArrow:
                if (newSelectedIndex - numTilesPerRow >= 0) newSelectedIndex -= numTilesPerRow;
                break;
            case KeyCode.Home:
                newSelectedIndex = 0;
                break;
            case KeyCode.End:
                newSelectedIndex = selectedDefinitions.Count - 1;
                break;
