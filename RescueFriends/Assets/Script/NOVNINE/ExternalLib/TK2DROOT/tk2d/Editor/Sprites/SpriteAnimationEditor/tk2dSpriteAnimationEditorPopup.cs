...
public class tk2dSpriteAnimationEditorPopup : EditorWindow
{
    ...
    void OnEnable()
    {
        ...
        menuItems.Add(new GUIContent("Clip"));
        menuTargets.Add(null);
    ...
    {
        get {
            return tk2dPreferences.inst.animListWidth;
        } set {
            tk2dPreferences.inst.animListWidth = Mathf.Max(value, minLeftBarWidth);
        }
    }
    ...
    void Commit()
    {
        ...
        foreach (tk2dSpriteAnimationClip clip in allClips) {
            ...
            if (names.Contains(clip.name)) {
                duplicateNames.Add(clip.name);
                dupNameString += clip.name + " ";
                continue;
            }
            names.Add(clip.name);
        }
