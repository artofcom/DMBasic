...
public class ClipEditor
{
    ...
    public void InitForNewClip()
    {
        selectClipNameField = true;
    ...
    {
        get {
            return tk2dPreferences.inst.animInspectorWidth;
        } set {
            tk2dPreferences.inst.animInspectorWidth = Mathf.Max(value, minInspectorWidth);
        }
    }
    ...
    {
        if (clipNameChangedEvent != null) clipNameChangedEvent(clip, 0);
    }
    void OnClipDeleted()
    {
        if (clipDeletedEvent != null) clipDeletedEvent(clip, 0);
    }
    bool OnClipSelectionChanged(int direction)
    {
        if (clipSelectionChangedEvent != null) clipSelectionChangedEvent(clip, direction);
        return clipSelectionChangedEvent != null;
    }
    void Repaint()
    {
    ...
    void HandleKeyboardShortcuts()
    {
        ...
        if (ev.type == EventType.KeyDown && GUIUtility.keyboardControl == 0) {
            ...
            case KeyCode.UpArrow:
                if (OnClipSelectionChanged(-1)) ev.Use();
                break;
            ...
            case KeyCode.Return:
                ...
                break;
            case KeyCode.F:
                if (preview != null) preview.ResetTransform();
                ev.Use();
                break;
            }
        }
