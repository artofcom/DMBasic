...
public abstract class tk2dBaseSprite : MonoBehaviour, tk2dRuntime.ISpriteCollectionForceBuild
{
    ...
    public bool FlipX
    {
        get {
            ...
            scale = new Vector3( Mathf.Abs(_scale.x) * (value?-1:1), _scale.y, _scale.z );
        }
    }
    ...
    public bool FlipY
    {
        get {
            ...
            scale = new Vector3( _scale.x, Mathf.Abs(_scale.y) * (value?-1:1), _scale.z );
        }
    }
    ...
    public bool SetSprite(string spriteName)
    {
        ...
        if (spriteId != -1) {
            ...
            Debug.LogError(gameObject.name+"] SetSprite - Sprite not found in collection: " + spriteName);
        }
        return spriteId != -1;
    }
    ...
    {
        get {
            if (null == this.collection || _spriteId < 0 || this.collection.Count <= _spriteId)
                return "";

            return this.collection.spriteDefinitions[_spriteId].name;
        } set {
            int spriteId = this.collection.GetSpriteIdByName(value);
    ...
    public tk2dSpriteDefinition spriteDefinition
    {
        get {
            ...
            return this.collection.spriteDefinitions[_spriteId];
        }
    }

    public void CopyTo(tk2dBaseSprite item)
    {
        item.collection = collection;
        item._color = _color;
        item._scale = _scale;
        item._spriteId = _spriteId;
