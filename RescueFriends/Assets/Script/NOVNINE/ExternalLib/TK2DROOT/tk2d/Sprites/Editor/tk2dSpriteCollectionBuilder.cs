...
public class tk2dSpriteCollectionBuilder
{
    ...
    static Texture2D ProcessTexture(tk2dSpriteCollection settings, bool additive, tk2dSpriteCollectionDefinition.Pad padMode, bool disableTrimming, bool isInjectedTexture, bool isDiced, Texture2D srcTex, int sx, int sy, int tw, int th, ref SpriteLut spriteLut, int padAmount)
    {
        ...
        if (numNotTransparent > 0) {
            ...
            if (customSpriteGeometry) {
                ...
                for (int x = 0; x < tw; ++x) if (ww[x] == 1) {
                        x0 = x;
                        break;
                    }
                for (int x = tw - 1; x >= 0; --x) if (ww[x] == 1) {
                        x1 = x;
                        break;
                    }
                for (int y = 0; y < th; ++y) if (hh[y] == 1) {
                        y0 = y;
                        break;
                    }
                for (int y = th - 1; y >= 0; --y) if (hh[y] == 1) {
                        y1 = y;
                        break;
                    }
            }
            ...
            case tk2dSpriteCollection.TextureCompression.Dithered16Bit_NoAlpha:
                tk2dEditor.TextureProcessing.FloydSteinbergDithering.DitherTexture(dtex, TextureFormat.ARGB4444, 0, 0, dtex.width, dtex.height);
                break;
            case tk2dSpriteCollection.TextureCompression.Dithered16Bit_Alpha:
                tk2dEditor.TextureProcessing.FloydSteinbergDithering.DitherTexture(dtex, TextureFormat.RGB565, 0, 0, dtex.width, dtex.height);
                break;
            }
            dtex.Apply();
    ...
    static void SetUpTargetTexture(tk2dSpriteCollection gen, Texture2D tex)
    {
        ...
        case tk2dSpriteCollection.TextureCompression.Uncompressed:
            targetFormat = TextureImporterFormat.AutomaticTruecolor;
            break;
        case tk2dSpriteCollection.TextureCompression.Reduced16Bit:
            targetFormat = TextureImporterFormat.Automatic16bit;
            break;
        case tk2dSpriteCollection.TextureCompression.Dithered16Bit_Alpha:
            targetFormat = TextureImporterFormat.Automatic16bit;
            break;
        case tk2dSpriteCollection.TextureCompression.Dithered16Bit_NoAlpha:
            targetFormat = TextureImporterFormat.Automatic16bit;
            break;
        case tk2dSpriteCollection.TextureCompression.Compressed:
        ...
        if (!gen.userDefinedTextureSettings) {
            if (importer.wrapMode != gen.wrapMode) {
                importer.wrapMode = gen.wrapMode;
                textureDirty = true;
            }
            if (importer.mipmapEnabled != gen.mipmapEnabled) {
                importer.mipmapEnabled = gen.mipmapEnabled;
                textureDirty = true;
            }
            if (importer.anisoLevel != gen.anisoLevel) {
                importer.anisoLevel = gen.anisoLevel;
                textureDirty = true;
            }
        }
