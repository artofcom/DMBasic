...
public class tk2dUIItem : MonoBehaviour
{
    ...
    private BoxCollider boxCollider
    {
        get {
            if(_boxcollider == null)
                _boxcollider = this.GetComponent<Collider>() as BoxCollider;
            return _boxcollider;
        }
    }
	...
	//amugana ( amugana@bitmango.com ) Last modified at : 2014.10.22 
	private float stationarySec;
	const float DELAYED_PRESS_THRESHOLD = 0.05f;

    public Vector3 downColliderSize
    {
        get {
            return colliderDownSize;
        }
    }
        ...
        if (!isPressed) {
            ...
            if (registerPressFromChildren || sentFromChild == null) {
                if (enabled) {
					isHoverOver = true;
					//amugana ( amugana@bitmango.com ) Last modified at : 2014.10.22 
					if (!isChildOfAnotherUIItem || isHoverEnabled) {
						DownUIItem(true);
					}
					else {
						isPressed = true;
						stationarySec = 0;
#if LLOG_TK2DUI
						Debug.Log("Press : delay check begin "+gameObject.name);
#endif
					}
                }
            }
    ...
    {
		//amugana ( amugana@bitmango.com ) Last modified at : 2014.10.22 
		if(enabled && isPressed && isChildOfAnotherUIItem && !isHoverEnabled) {
			if(stationarySec < DELAYED_PRESS_THRESHOLD) {
				if ((touch.phase == TouchPhase.Stationary) 
					|| (touch.phase == TouchPhase.Moved && touch.deltaPosition.magnitude < 0.02f)) 
				{
					stationarySec += tk2dUITime.deltaTime;
					if(stationarySec >= DELAYED_PRESS_THRESHOLD) {
#if LLOG_TK2DUI
						Debug.Log("Delayed Press : "+gameObject.name);
#endif
						isPressed = false;
						DownUIItem(true);
					}
    ...
    {
#if LLOG_TK2DUI
		Debug.Log("DownUIItem: "+gameObject.name+" isPressed:"+isPressed+"  updateCol:"+updateCollider);
#endif
		if(!isPressed) {
			isPressed = true;
			...
			if (parentUIItem != null) {
				parentUIItem.DownUIItem(updateCollider);
			}
		}
    }
    ...
    {
#if LLOG_TK2DUI
		Debug.Log("UpUIItem: "+gameObject.name+" isPressed:"+isPressed+"  updateCol:"+updateCollider);
#endif
		if(isPressed) {
			isPressed = false;
			...
			if (parentUIItem != null && upRoot != parentUIItem) {
				parentUIItem.UpUIItem(updateCollider, upRoot);
			}
		}
    }

	private void BeginTracking() {
		if(isHoverEnabled && boxCollider != null) {
			float buttonExpandThreshold = tk2dUIManager.buttonExpandThreshold;
			boxCollider.size += new Vector3(buttonExpandThreshold * 2, buttonExpandThreshold * 2, 0);
			colliderDownSize = boxCollider.size;
			boxCollider.center -= new Vector3(0,0,0.1f);
#if LLOG_TK2DUI
			Debug.Log(Time.time + "  P ] "+gameObject.name+"  z:"+boxCollider.center.z);
#endif
		}
	}
	...
	private void EndTracking() {
		if (isHoverEnabled && boxCollider != null) {
			...
			colliderDownSize = colliderSize;
#if LLOG_TK2DUI
			Debug.Log(Time.time + "  R ] "+gameObject.name+"  z:"+boxCollider.center.z);
#endif
		}
	}
        ...
        if (overUIItem != this) {
            if (isPressed) {
                ...
                if (!isUIItemChild) {
					newOverItem = null;
                    if (parentUIItem != null) {
                        newOverItem = parentUIItem.CurrentOverUIItem(overUIItem);
                    }
					UpUIItem(false, newOverItem);
                }
            } else {
                if (parentUIItem != null) {
                    newOverItem = parentUIItem.CurrentOverUIItem(overUIItem);
			...
			if(!isPressed && isHoverEnabled)
				DownUIItem(false);
		}

		if(!isHoverEnabled && newOverItem != this) {
#if LLOG_TK2DUI
			if(newOverItem != null)
				Debug.Log("CurrentOverUIItem : itemChange from "+this.name+"  to "+newOverItem.name);
			else
				Debug.Log("CurrentOverUIItem : itemChange from "+this.name+"  to null");
#endif
			return newOverItem;
		}
...
#endif
/*
            if (this.touch.phase == TouchPhase.Ended && isHoverEnabled && boxCollider != null) {
                boxCollider.center = colliderCenter;
#if LLOG_TK2DUI
                Debug.Log(Time.time + "  R ] "+gameObject.name+"  z:"+boxCollider.center.z);
#endif
...
*/
    ...
    public bool HoverOver(tk2dUIItem prevHover)
    {
        ...
        if (!isHoverOver) {
#if LLOG_TK2DUI
			Debug.Log("HoverOver : "+gameObject.name);
#endif
			DownUIItem(false);
			/*
            if (OnHoverOver != null) {
            ...
            if (OnHoverOverUIItem != null) {
                OnHoverOverUIItem(this);
            }
			*/
            isHoverOver = true;
        }
    ...
    public void HoverOut(tk2dUIItem currHoverButton)
    {
        if (isHoverOver) {
#if LLOG_TK2DUI
			Debug.Log("HoverOut : "+gameObject.name);
#endif
			UpUIItem(false);
			/*
            if (OnHoverOut != null) {
    ...
    public void SetColliderSize(float rate)
    {
        ...
        if (boxCollider != null) {
            boxCollider.size = new Vector3(colliderDownSize.x * rate, colliderDownSize.y * rate, boxCollider.size.z);
        }
    }
