...
public class tk2dUIManager : MonoBehaviour
{
    ...
    const float BUTTON_EXPAND_THRESHOLD_MILLIMETER = 9.0f;
    public static float buttonExpandThreshold = 0;

    const float SCROLL_BEGIN_THRESHOLD_MILLIMETER = 1.0f;
    public static float scrollBeginThreshold = 0;
...
#pragma warning restore 649
    ...
    private List<tk2dUIScrollableArea> scrollableAreas = new List<tk2dUIScrollableArea>();

    public void RegisterScrollableArea(tk2dUIScrollableArea scrollableArea)
    {
        if( !scrollableAreas.Contains(scrollableArea) ) 
        {
            scrollableAreas.Add(scrollableArea);
        }
    }

    public void UnRegisterScrollableArea(tk2dUIScrollableArea scrollableArea)
    {
        if( scrollableAreas.Contains(scrollableArea) )
        {
            scrollableAreas.Remove(scrollableArea);
        }

    }
    
    public bool IsalreadyScrolling(tk2dUIScrollableArea scrollableArea)
    {
        for( int i = 0; i < scrollableAreas.Count; ++i )
        {
            if( scrollableAreas[i] != scrollableArea && scrollableAreas[i].IsMovingScroll ) 
            {
                    return true;
            }
        }
    ...
    void Awake()
    {
        ...
        float oneMilliInWorldUnit = NNTool.ConvertMillimeterToWorldLength(Camera.main, 1);
        buttonExpandThreshold = oneMilliInWorldUnit * BUTTON_EXPAND_THRESHOLD_MILLIMETER;
		if(buttonExpandThreshold == 0)
			Debug.LogError("buttonExpandThreshold is "+buttonExpandThreshold+", make sure your mainCamera is properly set");
        scrollBeginThreshold = oneMilliInWorldUnit * SCROLL_BEGIN_THRESHOLD_MILLIMETER;
        tk2dUITime.Init();
    }
    ...
    void Start()
    {
        ...
        if (allCameras.Count == 0) {
            Debug.LogWarning("Unable to find any tk2dUICameras, and no cameras are connected to the tk2dUIManager. You will not be able to interact with the UI.");
        }
    }
    ...
    private void CheckInputs()
    {
        ...
        if (isPrimaryTouchFound || isSecondaryTouchFound) { //focus touch found
            ...
            if (resultTouch.phase == TouchPhase.Began) {
                ...
                if (pressedUIItem != null) {
                    pressedUIItem.UpdateTouch(resultTouch);
                    pressedUIItem = pressedUIItem.CurrentOverUIItem(hitUIItem);
                }
            }
                ...
                for (int t = 0; t < touchCounter; t++) {
                    ...
                    if (currTouch.fingerId == prevFingerID) {
                        ...
                        if (currTouch.phase == TouchPhase.Began) {
                            ...
                            prevPressedItem.UpdateTouch(currTouch);
							prevPressedUIItemList[f] = prevPressedItem.CurrentOverUIItem(currPressedItem);
                        }
                        break;
                    }
                }
    ...
    {
#if LLOG_TK2DUI
		Debug.Log("tk2dUIManager.OverrideClearAllChildrenPresses : "+item.name);
#endif
        if (useMultiTouch) {
            ...
            for (int n = 0; n < pressedUIItems.Length; n++) {
                ...
                if (tempUIItem!=null) {
                    if (item.CheckIsUIItemChildOfMe(tempUIItem)) {
                        pressedUIItems[n] = tempUIItem.CurrentOverUIItem(item);
                    }
                }
            }
        } else {
            if (pressedUIItem != null) {
                if (item.CheckIsUIItemChildOfMe(pressedUIItem)) {
                    pressedUIItem = pressedUIItem.CurrentOverUIItem(item);
                }
            }
        }
    }
