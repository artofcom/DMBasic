...
namespace AssetBundles
{
    ...
    public class AssetBundleManager : MonoBehaviour
    {
		...
		void Awake()
		{
			AssetBundleManager._instance = this;
		}
		...
		public static void SetSourceAssetBundleURL(string absolutePath, System.Int64 patchVersion)
		{
			if (!absolutePath.EndsWith("/"))
				absolutePath += "/";
			
			BaseDownloadingURL = absolutePath + patchVersion.ToString()+"/" + Utility.GetPlatformName() + "/";
		}
        ...
        static public AssetBundleLoadManifestOperation Initialize(string manifestAssetBundleName)
        {
            ...
            if (instancOBJ == null)
            {
                instancOBJ = new GameObject("AssetBundleManager", typeof(AssetBundleManager));
                DontDestroyOnLoad(instancOBJ);
            }
        ...
        {
            if (m_AssetBundleManifest == null) // network fail
                return null;
        ...
        // 현재 다운로드 중인 asset bundle의 목록 반환.
        public static void getDownloadProgress(ref Dictionary<string, float> outDicName_n_Progress)
        {
			if(m_InProgressOperations.Count <= 0)
                return;

            outDicName_n_Progress.Clear();
			for(int i = 0; i< m_InProgressOperations.Count; ++i)
			{
				AssetBundleDownloadOperation temp = m_InProgressOperations[i] as AssetBundleDownloadOperation;
				if(temp != null)
					outDicName_n_Progress[temp.assetBundleName] = m_InProgressOperations[i].Progress();
			}
        }
		...
		// by Mac
		/// <summary>
		/// Unloads all assetbundle and its dependencies.
		/// 메모리 
		/// </summary>
		static public void UnloadAllAssetBundle(bool removeAll = false)
		{
#if UNITY_EDITOR
			...
			if (SimulateAssetBundleInEditor)
				return;
#endif		
			Dictionary<string, LoadedAssetBundle>.Enumerator _enumerator = m_LoadedAssetBundles.GetEnumerator();
			
			while(_enumerator.MoveNext())
			{
				LoadedAssetBundle bundle = _enumerator.Current.Value;
				if(bundle != null)
				{
					bundle.m_AssetBundle.Unload(removeAll);
//					if (bundle.unload != null)
//						bundle.unload();
